...
- **Responsive panels** — Settings and Comments share one persistent preview surface: at 1050px or wider the Comments panel reserves space beside the document (Settings always overlays); narrower than that both overlay and narrow proportionally, so the document stays visible beside them; below 560px both dock to the bottom as a sheet so the document stays readable above. Only one panel is open at a time, and closing one returns focus to the button that opened it. If your OS is set to reduce motion, Penmark's own panel and highlight transitions skip animation.
