...
export default tseslint.config(
  ...
  // ADR 0001 boundary: src/core is platform-agnostic and must never import vscode.
  {
    ...
    rules: {
      "no-restricted-imports": [
        "error",
        { paths: [{ name: "vscode", message: "src/core is platform-agnostic (ADR 0001) — no vscode imports." }] },
      ],
    },
  },
  // The webview talks to the host via the versioned message protocol only.
  {
    ...
    rules: {
      "no-restricted-imports": [
        "error",
        { paths: [{ name: "vscode", message: "webview code communicates via the message protocol only (ADR 0001)." }] },
      ],
    },
  },
);
