...
- **Offer the local install — every time work becomes testable.** Penmark is sideloaded, not installed from a marketplace, so nothing the maintainer runs changes until a VSIX is packaged and installed into each IDE. Doing that by hand, across three IDEs, is the annoying part. So whenever a change is ready to try in a real IDE — a bug fixed, a feature finished, a branch about to be reviewed — **ask whether to run `npm run install:local`**, which packages and installs into every IDE found on the machine (VS Code, VS Code Insiders, Cursor, Windsurf, Antigravity) and reads the version back to confirm. Ask rather than install unprompted: a sideload swaps the extension under a running IDE, which is disruptive mid-task and not something to do without a say-so. Do not wait to be reminded, and do not hand over a copy-paste command instead of offering to run it. After a successful install, tell the maintainer to reload each IDE window — the extension host does not pick up the swap on its own.
