...
- **Branch and Workspace Isolation:** Always verify the active Git branch and workspace state at the very start of a session. You must unconditionally run the `using-git-worktrees` skill (or manually checkout a clean feature branch off `origin/main`) to ensure new feature work is isolated and does not mix with unrelated commits on existing branches.
